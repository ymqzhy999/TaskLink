...
// [TaskLink_uniapp/pages/index/index.vue]
...
const handleAICommand = async (rawText) => {
  try {
    ...
    if (jsonMatch) {
      try { commands = JSON.parse(jsonMatch[0]); } catch (e) { console.error(e); }
    }
    ...
    chatHistory.value.push({ role: 'ai', content: `🚀 已移交后台执行，请稍候（共 ${commands.length} 步）...` });
    scrollToBottom();

    // 一次性打包发送给后端
    uni.request({
      url: `${API_BASE}/api/phone/batch_run`,
      method: 'POST',
      data: { tasks: commands },
      timeout: 60000, // 🔥 修复 1：将超时时间延长至 60 秒，防止任务长时被挂断
      success: (res) => {
        if (res.data.code === 200) {
          // 🔥 修复 2：循环显示后端返回的具体步骤结果
          const results = res.data.data || [];
          results.forEach(item => {
            chatHistory.value.push({ 
              role: 'ai', 
              content: `步骤 ${item.step}: ${item.action} -> ${item.success ? '✅' : '❌ ' + item.msg}` 
            });
          });
          chatHistory.value.push({ role: 'ai', content: `✨ 任务全部执行完毕` });
        } else {
          chatHistory.value.push({ role: 'ai', content: `❌ 执行中断: ${res.data.msg}` });
        }
      },
      fail: (err) => {
        // 🔥 修复 3：细化错误提示
        let errMsg = '无法连接服务器';
        if (err.errMsg.includes('timeout')) errMsg = '任务执行超时，但后端可能仍在继续';
        chatHistory.value.push();
      },
      complete: () => {
        scrollToBottom();
